...
experiments = (expr_Yabe2020, expr_Miyazaki2013_uc, expr_Miyazaki2013_ue, expr_Boneh2014, expr_Kim2022, expr_Bernard2019, expr_Kumamoto2019) 
...
for ii in rng:
    ...
    scale = 4
    fig = plt.figure(figsize=(1*scale,0.9*scale))
#    ax = plt.gca()
    ax = plt.subplot(111)
    plt.subplots_adjust(left=0.15, bottom=0.15, right=0.98, top=0.975, wspace=0, hspace=0)
...
###
    ...
    cax = fig.add_axes([0.83 if IS_UC else 0.85, 0.185, 0.024, 0.34])
    plt.sca(ax)
    ...
    dl.plot_nlm_cases(ax, FSANNO, ms=7.5, show_circle=False, dy0=0.065, dx0_unidir=-0.09, dx0_planar=0.02, isolbl_above=True)

    kwargs_lbl = dict(ha='center', fontsize=FSANNO)
    ax.text(+0.300/norm, +0.150/norm, '{\\bf Single maximum}', color=dl.c_unidir, rotation=48, **kwargs_lbl)
    ax.text(-0.160/norm, +0.1/norm, '{\\bf Girdle}', color=dl.c_planar, rotation=-45, **kwargs_lbl)
#    ax.text(-0.250/norm, -0.320/norm, '{\\bf Unphysical}\n{\\bf eigenvalues}', color=sfplt.c_dgray, rotation=0, **kwargs_lbl)
    ...
    for jj,expr in enumerate(experiments):
        ...
        fillstyle = 'none' if expr['source']=='experiment' else 'full'
        strainzz = expr['Fzz'] - 1
        h = ax.scatter(x, y, c=strainzz, vmin=np.amin(ticks), vmax=np.amax(ticks), marker=mrk, s=8**2, facecolors='none', cmap=cmap)
        handles.append(h)
        titles.append(expr['plotname'])
#        ax.plot(x,y, ms=ms, markeredgewidth=mew, ls='none', color=expr['color'], fillstyle=fillstyle, marker=mrk, label=expr['plotname'], zorder=1+(len(experiments)-jj))

    legkwargs = {'handlelength':1.4, 'framealpha':1.0, 'fancybox':False, 'handletextpad':0.4, 'borderpad':0.37, 'edgecolor':'k'}
    legend_modellines = ax.legend([line_sdm,lines_drex]+handles, ['SDM','D-Rex']+titles, loc=2, bbox_to_anchor=(0.0,1.02), ncol=1, fontsize=FSLEG, frameon=False, **legkwargs)
    ...
    plt.ylabel(r'$\hat{%s}_4^0$'%(axname))
    ...
    fout = 'validation-plots/%s-%i.pdf'%(SELFNAME, ii)
    print('Saving %s'%(fout))
    plt.savefig(fout, transparent=True, dpi=250)
    plt.close()
    
#os.system('pdflatex validation-plots/state-space-validation.tex')
