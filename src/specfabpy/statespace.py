...
def plot_nlm_cases(ax, fs, ms=7.5, norm=1/np.sqrt(4*np.pi), \
                    ...
                    txt_unidir='Unidirectional', txt_planar='Planar'):
    ...
    plt.text(nl0_unidir[0]+dx0_unidir/norm, nl0_unidir[1]+dy, '{\\bf %s}'%(txt_unidir), color=c_unidir, **kwargs_lbl)

    ax.plot(*nl0_planar, c=c_planar, **kwargs_mrk)
    plt.text(nl0_planar[0]+dx0_planar/norm, nl0_planar[1]+dy, '{\\bf %s}'%(txt_planar), color=c_planar, **kwargs_lbl)
