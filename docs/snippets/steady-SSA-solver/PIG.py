...
cs = ax.tricontourf(triang, umag*ms2myr, levels=lvls, norm=colors.LogNorm(vmin=lvls[0], vmax=lvls[-1]), extend='both', cmap='inferno')
hcb = plt.colorbar(cs, cax=newcax(ax))
hcb.set_label('$u$ (m/yr)')
plt.savefig('%s-umag.png'%(domain['name']), **kw_save)

### Plot effective strain rate
...
cs = ax.tricontourf(triang, E_CAFFE, levels=lvls, norm=colors.LogNorm(vmin=lvls[0], vmax=lvls[-1]), extend='neither', cmap='PuOr_r')
hcb = plt.colorbar(cs, cax=newcax(ax))
