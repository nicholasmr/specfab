...
size = 1.6
fig = plt.figure(figsize=(3.8*size,2.0*size))
gs = fig.add_gridspec(ncols=2, nrows=1, width_ratios=[1,0.6])
plt.subplots_adjust(left=0.08, right=1-0.01, top=0.92, bottom=0.46, hspace=0.13)
...
sfplt.panellabel(ax_phi, 2, r'\textit{(a)}', frameon=False, fontsize=FS+1.5, bbox=(-0.155,1.25))
sfplt.panellabel(ax_J,   2, r'\textit{(b)}', frameon=False, fontsize=FS+1.5, bbox=(-0.195,1.25))
...
ax_phi.plot(np.NaN, np.NaN, '-', color='none', label=' ')
ax_phi.plot(np.NaN, np.NaN, '-', color='none', label=' ')
...
ax_phi.plot(f_gammaxz(drex_F)[I0:], f_shearang(drex_nlm)[I0:], ':', color=color_b, lw=lw, label=r'D-Rex')
...
I0 = 1 # plot only from this integratation timestep and onwards (shear angle not well defined before this time)
ax_phi.plot(f_gammaxz(sf_F_Lh)[I0:],  f_shearang(sf_nlm_Lh)[I0:],  '-',  c=color_b,   lw=lw, label=r'SDM, $L=%i$'%(Lh)) 
ax_phi.plot(f_gammaxz(sfd_F_Lh)[I0:], f_shearang(sfd_nlm_Ll)[I0:], '--', c=color_b,   lw=lw, label=r'SDM + $\mathcal{D}$, $L=%i$'%(Lh)) 
ax_phi.plot(f_gammaxz(sf_F_Ll)[I0:],  f_shearang(sf_nlm_Ll)[I0:],  '-',  c=c_orange,  lw=lw, label=r'SDM, $L=%i$'%(Ll))

### Labels, legends, ticks, etc.

ax_phi.set_ylabel(r'$\varphi$ ($^\circ$)')
ax_phi.set_xlabel(r'$\gamma_{xz}$', labelpad=1)

legkwargs = {'handlelength':1.4, 'framealpha':0, 'fancybox':False, 'handletextpad':0.4, 'borderpad':0.37, 'labelspacing':0.4, 'columnspacing':1.5, 'edgecolor':'k', \
            'borderaxespad':0, 'fontsize':FS-1.7}
handles,labels = ax_phi.get_legend_handles_labels()
hleg = ax_phi.legend(handles,labels, ncol=3, loc=3, bbox_to_anchor=(0.035,-0.01), bbox_transform=fig.transFigure, **legkwargs)
...
ax_phi.set_xticks(np.arange(0,12,2))
ax_phi.set_xticks(np.arange(0,12,1), minor=True)
...
if 1:
    ...
    plot_ODF_inset(0.16,0.12+y0, sf_nlm_trunc[I,:], lm_Ll, xyarr, title=r'SDM $(b)$', arr=np.array((0.3,1.5))*0.83)    

    I = I_sf(FxzHigh)
    xyarr = (sf_F_Ll[I,0,2], sf_shearang[I])
    plot_ODF_inset(0.28,0.05+y0, sf_nlm_trunc[I,:], lm_Ll, xyarr, title=r'SDM $(b)$', arr=np.array((-0.125,3.5))*1.05)

    I = I_drex(FxzHigh)
    xyarr = (drex_F[I,0,2], drex_shearang[I])
    plot_ODF_inset(0.38,0.01+y0, drex_nlm[I,:], lm_Ll, xyarr, title=r'D-Rex $(b)$', arr=np.array((+0.3,+4.5))*0.79)

### J-index panel

ax_J.plot(f_gammaxz(sf_F_Lh),  f_J(sf_nlm_Lh),  '-',  c=color_b, lw=lw, label=r'SDM')
ax_J.plot(f_gammaxz(sfd_F_Lh), f_J(sfd_nlm_Lh), '--', c=color_b, lw=lw, label=r'SDM + $\mathcal{D}$')
ax_J.plot(f_gammaxz(sf_F_Ll),  f_J(sf_nlm_Ll),  '-',  c=c_orange,  lw=lw, label=r'SDM, $L=%i$'%(Ll))

ax_J.plot(f_gammaxz(drex_F),  f_J(drex_nlm),  ':', c=color_b, lw=lw, label=r'D-Rex')
...
ax_J.set_xlabel(r'$\gamma_{xz}$', labelpad=1)
ax_J.set_ylabel(r'$J$')
...
ax_J.set_xticks(np.arange(0,12,2))
...
W = 0.15
ax1 = plt.axes([0.65, 0.72, W,W], projection='3d', zorder=2)
